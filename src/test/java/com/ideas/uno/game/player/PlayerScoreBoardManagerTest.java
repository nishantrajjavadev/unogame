...
public class PlayerScoreBoardManagerTest {
	...
	public void shouldReturnTrueIfPlayerScoreEqualToWinningScore() {
		Mockito.when(playerScoreBoard.getPlayers()).thenReturn(populatePlayers());
	...
	public void shouldReturnFalseIfPlayerScoreIsNotEqualToWinningScore() {
		Mockito.when(playerScoreBoard.getPlayers()).thenReturn(populatePlayersWithoutHighestScore());
		Mockito.when(playerScoreBoard.getWinningPoints()).thenReturn(500);
		boolean result = boardManager.isGameFinished();
	...
	@Test
	public void shouldReturnDefaultValueAsFalseForIsGameOverMethod() {
		Assert.assertFalse(boardManager.isGameOver());
	}
	
	@Test
	public void shouldReturnTrueWhenSetValue() {
		boardManager.setGameOver(true);
		Assert.assertTrue(boardManager.isGameOver());
	}
