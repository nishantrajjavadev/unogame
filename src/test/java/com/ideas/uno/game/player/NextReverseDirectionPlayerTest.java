...
public class NextReverseDirectionPlayerTest {
	...
	public void shouldReturnPreviousPlayerAsPerIterationWhenAnyPositionPlayerSearchedFromListOfPlayers() {
		...
		nextReverseDirectionPlayer =  new NextReverseDirectionPlayer(players.get(position), playerManager);
		...
		Assert.assertEquals(nextPlayer.getName(), players.get(position-1).getName());
		Assert.assertEquals(nextPlayer.getAge(), players.get(position-1).getAge());
		Assert.assertEquals(nextPlayer.getCards(), players.get(position-1).getCards());
	}
	...
	public void shouldReturnPlayerWhenFirstPlayerSearchedFromListOfPlayers() {
		...
		Assert.assertEquals(nextPlayer.getName(), players.get(players.size()-1).getName());
		Assert.assertEquals(nextPlayer.getAge(), players.get(players.size()-1).getAge());
		Assert.assertEquals(nextPlayer.getCards(), players.get(players.size()-1).getCards());
	}
	...
	public void shoudReturnPlayerFromListIfSearchedPlayerIsNotInList() {
		...
		nextReverseDirectionPlayer =  new NextReverseDirectionPlayer(playerOne, playerManager);
		Player nextPlayer = nextReverseDirectionPlayer.getNextPlayer();
		Collections.reverse(players);
		Assert.assertTrue(nextPlayer.equals(players.get(players.size()-1)));
		Assert.assertEquals(nextPlayer.getName(), players.get(players.size()-1).getName());
